...
namespace sdds
{
	...
	Reservation::Reservation(const std::string& reserv) {

		std::string temp = reserv;
	...
	std::ostream &operator<<(std::ostream &os, const Reservation& reservation) {
		...
		std::string email_bracket = "<" + reservation.m_email + ">";
		std::string tmp;
		...
		os << "Reservation " << reservation.m_reservId << ": "
			<< std::setw(10) << std::right << reservation.m_reservName << "  "
			<< std::setw(20) << std::left << email_bracket << "    "
			<< tmp << " "
			<< reservation.m_day << " @ "
			<< reservation.m_hour << ":00 for "
			<< reservation.m_peopleNum << " people."
			<< std::endl;
